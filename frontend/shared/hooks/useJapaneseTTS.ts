...
export const useJapaneseTTS = () => {
  ...
  // Audio element ref for Google TTS
    ...
    if (isSupported) {
      ...
      const loadVoices = () => {
        ...
        setState((prev: TTSState) => {
          ...
          if (prev.hasJapaneseVoices === hasJapaneseVoices && prev.availableVoices.length === japaneseVoices.length) {
            return prev;
          }
          return {
            ...prev,
        ...
        if (voices.length > 0 && japaneseVoices.length === 0) {
          ...
          setState((prev: TTSState) => {
            // Prevent infinite loop if already falling back
            if (!prev.hasJapaneseVoices && prev.availableVoices.length > 0) return prev;
            return {
              ...prev,
              isSupported: true,
  ...
  const speak = useCallback(
    (
      ...
      return new Promise<void>(resolve => {
        // Stop any currently playing speech (both Google and Browser)
        ...
        // ---------------------------------------------------------
        // GOOGLE TTS PROVIDER LOGIC
        // ---------------------------------------------------------
        if (ttsProvider === 'google') {
          const encodedText = encodeURIComponent(text);
          const googleUrl = `https://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&tl=ja&q=${encodedText}`;
          ...
          audio.play().catch(e => {
            if (hasFailed) return;
            hasFailed = true;
            console.warn('Google TTS play failed (likely autoplay policy), falling back', e);
            executeBrowserTTS();
          });
        ...
        // ---------------------------------------------------------
        // MICROSOFT EDGE TTS (ja-JP-NanamiNeural, High Quality)
        // ---------------------------------------------------------
        if (ttsProvider === 'edge') {
          const rate = options?.rate ?? 1.0;
          const rateOffset = rate >= 1 ? `+${Math.round((rate - 1) * 100)}%` : `-${Math.round((1 - rate) * 100)}%`;
          const encodedText = encodeURIComponent(text);
          const url = `/api/tts?text=${encodedText}&voice=ja-JP-NanamiNeural&rate=${encodeURIComponent(rateOffset)}`;
          ...
          audio.play().catch(e => {
            if (hasFailed) return;
            hasFailed = true;
            console.warn('Edge TTS play failed, falling back', e);
            executeBrowserTTS();
          });
        ...
        // ---------------------------------------------------------
        // BROWSER NATIVE TTS LOGIC (Fallback & Default)
        // ---------------------------------------------------------
        function executeBrowserTTS() {
          ...
          // Start browser TTS attempt
          attemptSpeak();
        };

        // If not Google or if Google fails, use browser TTS
        executeBrowserTTS();
      });
    },
    [isClient, pronunciationEnabled, ttsProvider],
  );
