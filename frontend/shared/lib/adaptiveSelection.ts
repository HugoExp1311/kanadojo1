...
export function createAdaptiveSelector(storageKey?: string) {
  ...
  const calculateWeight = (char: string, allChars: string[]): number => {
    ...
    // BUT: decays over time so mastered cards gradually reappear
    let masteryCooldown = 1;
    if (consecutiveCorrect >= 3) {
      const rawCooldown = Math.max(0.25, 1 - 0.15 * Math.min(consecutiveCorrect - 2, 5));

      // Decay: restore weight when card hasn't been seen for a while
      const DECAY_START_MS = 5 * 60 * 1000;   // Start restoring after 5 minutes
      const DECAY_FULL_MS = 30 * 60 * 1000;   // Fully restored after 30 minutes
      const timeSinceLastSeenMs = now - lastSeen;

      if (timeSinceLastSeenMs > DECAY_START_MS) {
        const decayProgress = Math.min(1, (timeSinceLastSeenMs - DECAY_START_MS) / (DECAY_FULL_MS - DECAY_START_MS));
        masteryCooldown = rawCooldown + (1 - rawCooldown) * decayProgress;
      } else {
        masteryCooldown = rawCooldown;
      }
    }
